...
| estado de domínio compartilhado entre views/páginas (dado vindo da API que mais de um lugar consome) | Pinia store em `src/stores/<dominio>.ts`, via `@stores` |
| regra de negócio (cálculo, validação, derivação sobre dado de uma ou mais stores) | composable em `src/composables/use<Algo>.ts`, via `@composables` |

Page e view importam as constantes de `src/data/<dominio>.ts` direto, via `@data`, **até a página ser conectada à API**. A camada de service já existe (`src/services/<dominio>.ts`, via `@services`) e encapsula as chamadas HTTP contra o backend descrito em `dashboard-api-routes.md` — axios (`src/services/http.ts`) para os recursos de domínio, `better-auth/vue` (`src/services/auth.ts`) para autenticação.

Pinia está instalado e registrado em `src/main.ts` (`createPinia()`, antes do `router`). Store de domínio entra em `src/stores/<dominio>.ts`, via `@stores`, como **setup store** (`defineStore('nome', () => { ... })`) — no mesmo estilo `<script setup>` do resto do projeto, não a sintaxe de options store. A store chama o service (`@services/<dominio>`) e expõe estado, getters e actions tipados; a view consome a store, nunca o service diretamente, quando o dado precisa sobreviver à troca de rota ou ser compartilhado entre mais de um lugar.

**Composable é onde mora regra de negócio** — cálculo, validação ou derivação que hoje ficaria solta num componente. Entra em `src/composables/use<Algo>.ts`, via `@composables`, e consome uma ou mais stores quando precisa de estado (`@stores/<dominio>`) — a página/view nunca implementa a regra, só chama o composable e usa o resultado. Ex.: `useAuth` (sessão derivada + mensagem única de recuperação de senha, RN-AU04), `useOnboardingStatus` (checklist de "bot pronto"), `useServiceTemplatePricing` (simulador de preço), `useAppointmentMetrics` (indicadores do Início), `useAppointmentCalendar` (estado de cada dia do calendário) — todos em `src/composables/`, implementando regras documentadas em `dashboard-api-routes.md`. Um composable que só repassa o estado de uma store sem nenhuma lógica em cima é redundante — nesse caso a view consome a store direto. Uma view que só precisa do resultado de **uma** chamada local de service, sem regra de negócio em cima, também não precisa de composable: chama a store/service direto no `onMounted`/handler.
